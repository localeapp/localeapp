  ...
  describe "#translations_url" do
    it "it extends the project_url" do
      with_configuration(@config) do
        @routes.translations_url.should == "http://test.host/api/projects/API_KEY/translations"
      end
    end

    it "adds query parameters on to the url" do
      with_configuration(@config) do
        @routes.translations_url(:query => {:updated_at => '2011-04-19', :foo => :bar}).should == "http://test.host/api/projects/API_KEY/translations?updated_at=2011-04-19&foo=bar"
      end
    end
  end
